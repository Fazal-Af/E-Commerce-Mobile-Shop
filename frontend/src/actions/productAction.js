...
export const getProduct =
  (keyword= "", currentPage = 1, price = [1000, 300000], category, ratings = 0) =>
  async (dispatch) => {
    try {
      dispatch({ type: ALL_PRODUCT_REQUEST });
      let link = ''
      if(Object.keys(keyword).length === 0){
         link = `/api/v1/products?keyword=${''}&page=${currentPage}&price[gte]=${price[0]}&price[lte]=${price[1]}&ratings[gte]=${ratings}`;
      }else{
      ...
      if (category) {
        if(Object.keys(keyword).length === 0){
          link = `/api/v1/products?keyword=${''}&page=${currentPage}&price[gte]=${price[0]}&price[lte]=${price[1]}&category=${category}&ratings[gte]=${ratings}`;
        }else{
